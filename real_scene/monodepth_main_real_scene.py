...
import time
import argparse
from monodepth_model_real_scene import *
from tensorflow.python import pywrap_tensorflow
import cv2

from dataloader_real_scene import *
...
filenames_file = '/home/fufu/work/code_UnLFdisp/real_scene/train0310.txt'  # train  filename_file path
# filenames_file = '/home/jethong/PycharmProjects/multi_input/real_scene/train_all_lytro.txt'  # train  filename_file path
filenames_fileTest = '/home/fufu/work/code_UnLFdisp/real_scene/train0310.txt'  # test or val
...
# data_path = curdir + "/evaluation_toolkit/data"

output_directory = './all_real_data/0310/'  # save the test result into this dir
log_directory = output_directory
checkpoint_path = '/home/fufu/work/code_UnLFdisp/real_scene/all_real_data/0117/monoLFdepth/model-5000'
...
def test_pp(params, times):
    ...
    for step in range(num_test_samples):
        two_disp = sess.run(model.two_centerdisp)
        # disp, disp0 = sess.run(model.center_disp, model.disp_center_est)
        disp = two_disp[0]
        disp0 = two_disp[1]
    ...
    for i in range(disparities.shape[0]):
        ...
        evalpath = output_directory + "/" + str(times)
        for k in range(8):
            cv2.imwrite(evalpath + '/' + scenename + '{}.png'.format(str(k)), (img[i][k] * 255.0).astype(np.uint8))
