...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, mode):
        ...
        if mode == 'train':
            ...
            # random flip up down
            imagecenter = tf.cond(not_do_fliplr > 0.5, lambda: tf.image.flip_left_right(imagecenter_o), lambda: imagecenter_o)
            ...
            imagepie = tf.cond(not_do_fliplr > 0.5, lambda: [tf.image.flip_left_right(single) for single in imagena_o],
                               lambda: imagepie_o)
            imageheng = tf.cond(not_do_fliplr > 0.5, lambda: [tf.image.flip_left_right(single) for single in imageheng_o_inv],
                                lambda: imageheng_o)
